...
if ($mac) {
    ...
    if ($mac -eq $false) {
        ...
        if ($v6 -eq $true) {
            $current_lease = Get-DhcpServerv6Scope | Get-DhcpServerv6Lease | Where-Object ClientId -eq $mac
        }
        else {
...
if ($ip -and (-not $current_lease)) {
    if ($v6 -eq $true) {
        $current_lease = Get-DhcpServerv6Scope | Get-DhcpServerv6Lease | Where-Object IPAddress -eq $ip
    }
    else {
...
if ($state -eq "absent") {
    ...
    if ($current_lease_exists -eq $false) {
        ...
        if ($current_lease_reservation -eq $true) {
            ...
            Try {
                if ($v6 -eq $true) {
                    $current_lease | Remove-DhcpServerv6Reservation -WhatIf:$check_mode
            ...
            Try {
                if ($v6 -eq $true) {
                    $current_lease | Remove-DhcpServerv6Lease -WhatIf:$check_mode
                    $state_absent_removed = $true
                }
                else {
...
if ($state -eq "present") {
    ...
    if (($current_lease_reservation -eq $false) -and ($current_lease_exists -eq $true)) {
        if ($type -eq "reservation") {
            Try {
                ...
                if ($v6 -eq $true) {
                    $current_lease | Add-DhcpServerv6Reservation -WhatIf:$check_mode
                }
                else {
                ...
                if (-not $check_mode) {
                    if ($v6 -eq $true) {
                        $current_reservation = Get-DhcpServerv6Lease -ClientDuid $params.ClientId -Prefix $current_lease.ScopeId
                    }
                    else {
                ...
                if ($v6 -eq $true) {
                    $current_reservation | Set-DhcpServerv6Reservation @params -WhatIf:$check_mode
                }
                else {
                ...
                if (-not $check_mode) {
                    if ($v6 -eq $true) {
                        $updated_reservation = Get-DhcpServerv6Lease -ClientDuid $params.ClientId -Prefix $current_reservation.ScopeId
                    }
                    else {
    ...
    if (($current_lease_reservation -eq $true) -and ($current_lease_exists -eq $true)) {
        if ($type -eq "lease") {
            Try {
                ...
                Try {
                    if ($v6 -eq $true) {
                        Add-DhcpServerv6Lease @lease_params -WhatIf:$check_mode
                    }
                    else {
                ...
                if (-not $check_mode) {
                    Try {
                        if ($v6 -eq $true) {
                            $new_lease = Get-DhcpServerv6Lease -ClientDuid $lease_params.ClientId -Prefix $lease_params.ScopeId
                        }
                        else {
        ...
        if ($type -eq "reservation") {
            ...
            # Update the reservation with new values
            if ($v6 -eq $true) {
                $current_lease | Set-DhcpServerv6Reservation @params -WhatIf:$check_mode
            }
            else {
            ...
            if (-not $check_mode) {
                if ($v6 -eq $true) {
                    $reservation = Get-DhcpServerv6Lease -ClientDuid $current_lease.ClientId -Prefix $current_lease.ScopeId
    ...
    if ($current_lease_exists -eq $false) {
        ...
        Try {
            ...
            if (-not $check_mode) {
                if ($v6 -eq $true) {
                    $new_lease = Get-DhcpServerv6Lease -ClientDuid $mac -Prefix $scope_id
                    $module.Result.lease = Convert-ReturnValue -Object $new_lease
                }
                else {
            ...
            if ($type -eq "reservation") {
                ...
                Try {
                    if ($check_mode) {
                        ...
                        if ($v6 -eq $true) {
                            Add-DhcpServerv6Reservation -Prefix $scope_id -ClientDuid $mac -IPAddress $ip -WhatIf:$check_mode
                        }
                        else {
                        ...
                        if ($v6 -eq $true) {
                            $new_lease | Add-DhcpServerv6Reservation -WhatIf:$check_mode
                        }
                        else {
                ...
                if (-not $check_mode) {
                    ...
                    if ($v6 -eq $true) {
                        $new_lease = Get-DhcpServerv6Reservation -ClientDuid $mac -Prefix $scope_id
                        $module.Result.lease = Convert-ReturnValue -Object $new_lease
                    }
                    else {
