...
function Get-Gplink {
    ...
    if ($null -eq $gplink) {
        Write-DebugLog "Either Invalide DN in the current domain, specify a DC of the target DN domain or no GPOlinked to this DN"
    }
...
$global:log_path = $log_path = $module.Params.log_path
            ...
            foreach($pa in $path)
            {
                ...
                if ($path) {
                    ...
                    if( $gpoalllinks.DisplayName -notcontains $gponame) {
                        ...
                        Write-DebugLog "creating GPO Link"
                    ...
                    if(($gpoonpath.Enforced -match $gposetparam.Enforced) -and ($gpoonpath.Enabled -match $gposetparam.LinkEnabled) -and (($null -eq $gposetparam.order) -or ($gpoonpath.Order -eq $gposetparam.order))) {
                        ...
                        #gplink needs update
                        Write-DebugLog "changing GPO Link"
        ...
        if ($path) {
            ...
            if($gpos.Displayname -contains $gponame) {
                ...
                if($gpoonpath.DisplayName -contains $gponame ) {
                    Write-DebugLog "Remove GPO Link"
